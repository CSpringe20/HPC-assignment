...
int main(int argc, char **argv)
{
  ...
  int  Niterations;   // basically time
  int  periodic;      // to know what to do at the border
  int  S[2];          // to store grid size nx ny
  
  int     Nsources;   // number of pixels acting like heat sources, where we inject (set) temperature
  int    *Sources;    // probably the pointer to the location on the map of the sources
  double  energy_per_source;   // how much temp to add to the sources

  double *planes[2];  // two pointers for the two planes (before upd and after, old and new)
  
  double injected_heat = 0; // amount of heat injected in system, to be able to check if energy preserved 

  int injection_frequency;  // i think at what frame rate to inject temp
  int output_energy_at_steps = 0; // idk
   
  /* argument checking and setting */
  initialize ( argc, argv, &S[0], &periodic, &Niterations,
	       &Nsources, &Sources, &energy_per_source, &planes[0],
	       &output_energy_at_steps, &injection_frequency );  // take argvs and initialize framework with them
  
  
  int current = OLD; // current plane is old one?, OLD = 0 so just select the first ptr in array planes

  if ( injection_frequency > 1 ) // if we inject more rarely than once a frame then start injecting now to not waste time
    inject_energy( periodic, Nsources, Sources, energy_per_source, S, planes[current] );
  
  for (int iter = 0; iter < Niterations; iter++) // iteration through time
		 ...
		 )
{
  ...
  *injection_frequency = *Niterations;
  ...
  while ( 1 )
  {
	...
	switch( opt )
	  ...
	  case 'h': printf( "valid options are ( values btw [] are the default values ):\n"
			    ...
			    "-E    how much energy per ssource source [1.0]\n"
   ...
   *       MAYBE IMPOSE MIN X,Y; MIN SOURCE 1; MIN ENERGY 1
   */
  ...
  //
  ret = memory_allocate( S, planes ); // now we allocated the 2 contigous planes in mem and init them to 0 (cold start)
 ...
 */
{
  ...
  memset ( planes_ptr[OLD], 0, 2*bytes*sizeof(double) );  // init both planes to 0, cold start
  planes_ptr[NEW] = planes_ptr[OLD] + bytes; // the two planes are contigous in memory
  ...
  for ( int s = 0; s < Nsources; s++ )
    {
      ...
      (*Sources)[s*2+1] = 1+ lrand48() % size[_y_];  // multiple sources could overlap but its fine coz we sum their heat
    }
